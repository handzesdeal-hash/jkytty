...
// If embedded in the host site, propagate balance changes to host so the site's
// user balance (meProfile) stays in sync. The host exposes `plinkoSetBalance`.
if (typeof window !== 'undefined') {
  const winAny = window as any;
  let isFirstBalanceEmission = true;
  balance.subscribe((val) => {
    try {
      // Ignore the very first synchronous emission (DEFAULT_BALANCE)
      // to avoid overriding host balance when entering embedded mode.
      if (isFirstBalanceEmission) {
        isFirstBalanceEmission = false;
        return;
      }
      ...
      if (typeof winAny.plinkoSetBalance === 'function') {
        ...
        const res = winAny.plinkoSetBalance(val);
